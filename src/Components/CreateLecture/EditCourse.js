...
export default function EditCourse(){
    let curCourIndex = JSON.parse(localStorage.getItem('currentCourseIndex')); 
    let curCour = JSON.parse(localStorage.getItem('currentCourseEdit'));
    //Сам курс
    ...
    function handleViewCourse() {
        ...
        if (crs.Name === '') {
            ...
            crs.active = false;
            setCurrentCourse(crs);
            console.log(crs.active);
        } else if (crs.lecture.length < 1) {
            alert('В курсе должен быть хотя бы 1 урок');
            crs.active = false;
            setCurrentCourse(crs);
        } else {
            crs.active = !crs.active;
            setCurrentCourse(crs);
        }
    }
    ...
    function handleSubmitCourse() {
        let thisCourse = [...currentCourse];
        let allCrs = JSON.parse(localStorage.getItem('allCourse'));
        allCrs[curCourIndex] = thisCourse;
        ...
        <div>
            <div className='shadow w-80 m-5 p-5'>
                <div className='d-flex justify-content-between align-items-center mb-5'>
                    <input value={currentCourse.Name} onChange={(e) => changeNameCourse(e.target.value)} style={{ border: 'none', 'borderBottom': '1.5px solid #d36a92' }} maxLength='30' className='nameCourse' type='text' placeholder='Название' />
                    <div className='d-flex align-items-center'>
                        <h5 style={{ 'marginRight': '5px' }}>Отображать на сайте</h5>
                        <Switch checked={currentCourse.active} onChange={() => handleViewCourse()} />
                    </div>
                </div>
                ...
                <textarea value={currentCourse.description} onChange={(e)=> changeDescriptionCourse(e.target.value)} className='w-100 mb-5' type='text' placeholder='Описание Курса' />
            ...
            <div className="w-100 d-flex justify-content-center mb-5">
                <Button href='/mycourse' disabled={(currentCourse.Name === '')? true : false} onClick={()=> handleSubmitCourse()} variant="success">Редактировать курс</Button>
            </div>
